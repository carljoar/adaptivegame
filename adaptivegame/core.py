...
class AdaptiveGame(object):
    def __init__(self, num_iters, y0, eps, C):
        ...
        self.convergence = np.zeros(num_iters)
        self.e_history = np.zeros(num_iters)
        self.mca_history = np.zeros(num_iters)
        self.h2_history = np.zeros(num_iters)
    ...
    def run(self):
        ...
        while runTrue and MCA(y)<(C-0.45e-9) and k<num_iters:
            ...
            if eps < 1e-10:
                runTrue = False
            self.e_history[k] = E(y,y_prev)
            self.mca_history[k] = MCA(y)
            self.h2_history[k] = np.sum(y*y)
            self.convergence[k] = np.sum(y)
            k = k+1
